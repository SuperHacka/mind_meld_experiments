...
def welcome(request, responder):
    ...
    try:
        ...
        prefix = 'Hello welcome to the application, {name}. '
    except KeyError:
        prefix = 'Hello. '
...
form_payment_order = {
    'entities':[
        FormEntity(
            entity='cuisine',
            # role='payment_from',
            responses=['Sure. What cuisine you want'],
            retry_response=['Cuisine type does not exist'],
            ),
        FormEntity(
            entity='dish',
            # role='payment_to',
            responses=['Ok,how many portion would you like?'],
            retry_response=['Dish requested does not exist'],
            ),
        ],
        'max_retries': 2,
        'exit_keys': [
        ...
        'exit_msg': "Sorry the transfer cannot be made at the moment"
}


@app.auto_fill(intent='start_over', form=form_payment_order)
def payment_order(request, responder):
    """
    Trigger at the end of the order, where user have to pay for the order
    """
    cuisine_var = None
    payment_entities = next((e for e in request.entities if e['type'] == 'cuisine'),None)
    cuisine_var = app.question_answerer.get(index='cuisine',id=payment_entities['value'])[0]
    for entity in request.entities:
        if entity['type'] == ['cuisine']:
            responder.slots['cuisine'] == entity['cuisine']

            # replies = ['Thanks for the order, please wait sometime for it {cuisine} to be ready']

        if cuisine_var:
            responder.slots['cuisine'] = cuisine_var['cuisine']
            responder.reply('Thanks for the order, please wait sometime for it {cuisine} to be ready')
            return
    # responder.reply('Thanks for the order, please wait sometime for it {cuisine} to be ready')
